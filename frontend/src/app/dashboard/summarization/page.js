...
function Summarization() {
  ...
  async function submitDocument() {
    ...
    setLoading(true);
    try {
      const response = await PublicApi.post("api/summarize", {
        text: document,
      });
  ...
  return (
    <div className="p-6 space-y-6">
      {loading && (
        <div className="fixed inset-0 opacity-50 bg-black flex justify-center items-center py-10">
          <div className="w-20 h-20 border-4 border-blue-500 border-dashed rounded-full animate-spin"></div>
        </div>
      )}

      <h2 className="text-3xl text-center">Text Summarization</h2>
